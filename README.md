...
Streamlit Community Cloud is the best fit because the interface is already built with Streamlit, the repository is public, and the demo needs no database or secret.
...
`render.yaml` contains the required Blueprint configuration:
...
Render is suitable when a general web-service container is preferred, but Streamlit Community Cloud is simpler for this portfolio application.

## 11. How to explain it to a recruiter

### 30-second explanation

> CreditWise is an end-to-end responsible-ML portfolio project built with Python, scikit-learn, and Streamlit. I validated 1,000 loan records, used a leakage-safe preprocessing pipeline, reserved an untouched 190-row final holdout, and compared three models only on the 760-row development set. Gradient Boosting achieved 96.32% holdout accuracy, 94.40% F1, and 98.71% ROC-AUC. I also excluded sensitive features, added input guardrails, subgroup diagnostics, tests, CI, reproducible reports, and a live web interface.

### Likely technical questions

1. **Why not report only accuracy?** The data has 652 rejected and 298 approved labelled records. Precision, recall, F1, ROC-AUC, and the confusion matrix expose different error types.
2. **How did you prevent leakage?** Imputation, scaling, and one-hot encoding are fitted inside the pipeline separately for every training fold.
3. **Why is the holdout untouched?** Candidate models are compared only on the development set; the holdout is used once for final evaluation.
4. **Why Gradient Boosting?** It produced the best development-CV F1 and the strongest overall metric balance among the three tested models.
5. **Why remove Age?** It is sensitive in lending and provided no holdout-performance benefit. Removing it also slightly improved development-CV results.
6. **Is the displayed score a probability?** It is an uncalibrated model score. Calibration and external validation are required before interpreting it as a real-world probability.
7. **Does removing sensitive columns guarantee fairness?** No. Proxy variables and subgroup outcomes still require formal auditing and monitoring.
